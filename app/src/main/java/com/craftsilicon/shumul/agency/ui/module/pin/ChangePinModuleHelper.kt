...
class ChangePinModuleResponse(
    ...
    init {
        try {
            AppLogger.instance.appLog("PIN:RESPONSE:ENCRYPTED", Gson().toJson(response))
...
fun changePin(
    ...
    return try {
        ...
        val map = APP.data(
            context = context,
            storage = model.preferences,
            action = ActionTypeEnum.CHANGE_PIN.action,
            uniqueId = uniqueID
        )
        ...
        map["CALLTYPE"] = "O-"
        AppLogger.instance.appLog("PIN:REQUEST", Gson().toJson(map))
        PayloadRequest(
            uniqueId = "$unique",
