...
@Composable
fun AccountStatement(data: GlobalData) {
    ...
    CompositionLocalProvider(LocalLayoutDirection provides layoutDirection()) {
        Scaffold(topBar = {
            TopAppBar(
                title = {
                    Text(
                        text = stringResource(id = R.string.account_statement_),
            ...
            Column(
                ...
                NavHost(
                    ...
                    startDestination = Module.AccountStatement.Full.route
                ) {
                    setComposable(route = Module.AccountStatement.Full.route) {
                        BackHandler(enabled = true) {
                            data.controller.navigateUp()
                        }
                        AccountFullStatementModule(action = {
                            data.controller.navigateUp()
                        })
                    }

                    setComposable(route = Module.AccountStatement.Mini.route) {
                        BackHandler(enabled = true) {
                            data.controller.navigateUp()
                        }
                        AccountMiniStatementModule(action = {
                            data.controller.navigateUp()
                        })
                    }
                }
